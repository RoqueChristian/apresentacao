...
METAS_VENDEDORES = {
    ...
    "DENIS SOUSA": 930000.00,
    "THIAGO SOUSA": 400000.00,
    "JOECIA": 70000.00,
    "NATALIA SILVA": 200000.00
}
...
def aplicar_filtros(df, vendedor='Todos', mes=6, ano=None, situacao='Faturada'):
    df_filtrado = df.copy()
    if ano is None:
...
def renderizar_pagina_vendas(df):
   ...
   # Exibir todos os gráficos
    st.plotly_chart(criar_grafico_vendas_diarias(df_filtrado, mes_atual, ano_atual))
    st.plotly_chart(fig_performance)
    st.plotly_chart(exibir_grafico_ticket_medio(df_ticket_medio))
    st.plotly_chart(criar_grafico_barras(produtos_mais_vendidos(df_filtrado), 'Descricao_produto', 'Valor_Total_Item', 'Top 10 Produtos Mais Vendidos', {'Descricao_produto': 'Produto', 'Valor_Total_Item': 'Valor Total de Venda'}))
    st.plotly_chart(criar_grafico_barras_vendas_linha(df_filtrado))
